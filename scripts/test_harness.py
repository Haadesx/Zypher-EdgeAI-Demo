...
class QEMUTestHarness:
    ...
    def test_startup(self) -> TestResult:
        ...
        if found >= 2:
            result.passed = True
            result.message = f"Found {found}/{
                len(startup_patterns)} startup markers"
        else:
            result.message = f"Only found {found}/{
                len(startup_patterns)} startup markers"
    ...
    def test_sensor_sampling(self) -> TestResult:
        ...
        if found >= 2:
            result.passed = True
            result.message = "Sensor subsystem working"
        else:
    ...
    def test_gesture_detection(self) -> TestResult:
        ...
        if len(gestures_found) >= 2:
            result.passed = True
            result.message = f"Detected {len(gestures_found)}/4 ({coverage:.0f}%) gesture types: {gestures_found}"
        else:
            result.message = f"Only detected {
                len(gestures_found)} gesture type(s)"
